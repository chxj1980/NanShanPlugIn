...
vector<GRGPlayHandle::PLAYERINFO> GRGPlayHandle::m_vPlayChan;
char * GRGPlayHandle::mFileName = NULL;
int GRGPlayHandle::m_PlayPort = -1; //用于静态函数中

CRITICAL_SECTION GRGPlayHandle::m_Lock;

GRGPlayHandle::GRGPlayHandle(void)
{
    m_nDecodeType = 0 ;
...
bool GRGPlayHandle::initialPlayer( int nPort,int nDecodeType,BYTE* buff, int nSize, int nStreamType )
{
...
bool GRGPlayHandle::startPlayer( HWND hWnd )
{
...
void  GRGPlayHandle::DrawFunCallBack(long nPort,HDC hDc,void *nUser)
{
	GRGPlayHandle* pHandler = (GRGPlayHandle*)nUser;
...
bool GRGPlayHandle::startPlayer2( HWND hWnd, LPDRAWCBCALLLBACK lpDrawCBFun, void *pUser )
{
...
bool GRGPlayHandle::startPlayer4Standard( HWND hWnd,LPDecCBFun lpDecCBFun, int nCameraID )
{
	if (hWnd)
	{
		return true;
...
bool GRGPlayHandle::addData( BYTE* buff, int nSize )
{
...
bool GRGPlayHandle::controlPTZSpeed(int nCmd, bool bEnable,int nSpeed)
{
	return controlPTZSpeedWithChannel(nCmd, bEnable, nSpeed, 0);
}

bool GRGPlayHandle::controlPTZSpeedWithChannel(int nCmd, bool bEnable, int nSpeed, int nChannel)
{
	if (nCmd == 24)
	{
		m_nSrcWind = nSpeed;
...
bool GRGPlayHandle::locPlay_OpenFile( int nPort,int nDecodeType, HWND hWnd,char* fileName )
{
...
bool GRGPlayHandle::locPlay_Control( int nCmd,int nInValue,int* outValue )
{
	char szLog[256] = {0};
...
bool GRGPlayHandle::locPlay_Stop()
{
...
bool GRGPlayHandle::convertToBmpFile( char * pBuf,LONG nSize,LONG nWidth,LONG nHeight,LONG nType,char *sFileName )
{
...
bool GRGPlayHandle::oneByOneBack( LONG nPort )
{
    bool bRe = (bool)PLAY_OneByOneBack(nPort);
    return bRe;
}

bool GRGPlayHandle::oneByOne( LONG nPort )
{
    bool bRe = (bool)PLAY_OneByOne(nPort);
    return bRe;
}

bool GRGPlayHandle::setPlayedTimeEx( LONG nPort,DWORD nTime )
{
    bool bRe = (bool)PLAY_SetPlayedTimeEx(nPort,nTime);
    return bRe;
}

void GRGPlayHandle::reSetDDrawDevice( HWND hWnd )
{
    PLAY_RefreshPlay(m_nPlayerPort);
}

bool GRGPlayHandle::controlCameraSound( bool bOpenSound )
{
    if(bOpenSound)
	{
		m_bOpenSound = true;
...
intptr_t GRGPlayHandle::openVideoFile( char *sFileName )
{
    m_bDvrFile = false;
    long lPort = -1;
...
int GRGPlayHandle::readVideoFile( char *buff,int len, int *streamType )
{
    DWORD nRealRead = 0;
    if(INVALID_HANDLE_VALUE == m_hStreamFile)
    {
...
bool GRGPlayHandle::closeVideoFile()
{
    m_read = false;
    if(m_hStreamFile)
    {
...
bool GRGPlayHandle::controlPlay( int nCmd,int param, int *outValue )
{
...
void GRGPlayHandle::ClearPlayBuffer()
{
    if (m_nPlayerPort>=0)
    {
        PLAY_ResetSourceBuffer(m_nPlayerPort);		
...
void GRGPlayHandle::pusePlayDecode( bool bPuse )
{
    if (m_nLocPlayPort>=0)
    {
        if (bPuse)
...
bool GRGPlayHandle::setFileRefCallBack( LONG nPort,void (__stdcall *pFileRefDone) (DWORD nPort,void *nUser),void *nUser )
{
...
{
	GRGPlayHandle *pHandle = (GRGPlayHandle *)nReserved;
	char szType[256] = "DH";
	nReserved = szType;
...
bool GRGPlayHandle::setDisplayCallBack( LONG nPort, void (__stdcall *pFileRefDone)(long nPort,char * pBuf,long nSize,long nWidth,long nHeight,long nStamp,long nType,void *nReserved) )
{
	BOOL bRe;
...
bool GRGPlayHandle::capturePicture( char* sFilePath )
{
...
void CALLBACK GRGPlayHandle::DecCBFun4Standard( long nPort,char * pBuf,long nSize,FRAME_INFO * pFrameInfo, void *nReserved1,long nReserved2 )
{
    int nChanID = -1;
...
void CALLBACK GRGPlayHandle::DecCBDecode(LONG nPort, FRAME_DECODE_INFO* pFrameDecodeInfo, FRAME_INFO_EX* pFrameInfo, void* pUser)
{
	int nChanID = -1;
	FRAME_INFO_D mFrameInfo;
...
void CALLBACK GRGPlayHandle::DecCBFunc( DWORD nPort,void *user )
{
    GRGPlayHandle * phand = (GRGPlayHandle *) user;
}

bool GRGPlayHandle::initialPlayerEx( PINITPY_PARAM pIParam )
{
    if (NULL == pIParam)
    {
        return false;
...
bool GRGPlayHandle::startPlayer4StandardEx( PSTARTPYS_PARAM pParam )
{
    if (NULL == pParam)
    {
        return false;
...
bool GRGPlayHandle::locPlay_OpenFileEx( PLOCPY_OPENF_PARAM_PARAM pParam )
{
	char szLog[256] = {0};
...
bool GRGPlayHandle::SetCropArea( bool bFlag, int x1, int y1, int x2, int y2)
{
	BOOL bRet = FALSE;
	DISPLAYRECT rct;
	rct.left = x1;
...
bool GRGPlayHandle::SetDecodeParam( int nCmd, bool bFlag, int x1, int y1, int x2, int y2)
{
	BOOL bRet = FALSE;
	int nModuleType = 0;
...
bool GRGPlayHandle::SetYUVCallBack( LPYUVCALLBACK pCallBack)
{
	return true;
}
bool GRGPlayHandle::SetStreamSaveTime( int nSaveTime)
{
	return true;
}
bool GRGPlayHandle::PlayBackSaveStream( HWND hWnd)
{
	return true;
}
bool GRGPlayHandle::StopPlayBackSaveStream( )
{
	return true;
}

//2013 0927 pzm add
bool GRGPlayHandle::SetRenderMode( int nMode)
{
	return false;
}

bool GRGPlayHandle::SetPosByFrameNum(DWORD nFrameNum)
{
	FRAME_POS pos;
...
bool GRGPlayHandle::GetVideoCut( DWORD beginFrame, DWORD endFrame, LPSTR filename)
{
...
bool GRGPlayHandle::StopCutVideo()
{
	return false;
}

struct GRGPlayHandleS
{
	GRGPlayHandle *pHandle;
	void *pUser;
};

void GRGPlayHandle::pDHFileOverCB(DWORD nPort, void *pUser)
{
	GRGPlayHandleS *pInfo = (GRGPlayHandleS *)pUser;
	if(pInfo != NULL)
	{
...
bool GRGPlayHandle::SetFileEndCallback( void *pUser,pFileOverCB pCallBack)
{
	GRGPlayHandleS *pInfo = new GRGPlayHandleS;
...
bool GRGPlayHandle::GetMediaInfo(MediaInf &mInf)
{
	return false;
}

bool GRGPlayHandle::GetDecodeParam( int nCmd, int &x1, int &y1, int &x2, int &y2)
{
	BOOL bRet = FALSE;
	int nModuleType = 0;
	switch (nCmd)
...
int  GRGPlayHandle::AllpurposeI(int nCmd,char *pInPutPara,char *pOutPutPara)
{
