...
#else
// 异常回调函数
typedef  IVS_VOID (*NET_EXCEPTION_CALLBACK)(IVS_ULONG  ulChannel,     // 通道号;
                                        IVS_INT32  iMsgType,      // 异常消息类型;
                                        IVS_VOID*  pParam,        // 对应异常的信息，可扩展为结构体指针;
                                        IVS_VOID*  pUser);        // 用户数据;

// 媒体流回调
typedef  IVS_VOID (*NET_DATA_CALLBACK)(IVS_CHAR    *pBuf,      // 回调数据;
                                IVS_UINT32  uiSize,     // 数据长度;
                                IVS_VOID    *pUser);    // 用户数据;
#endif
