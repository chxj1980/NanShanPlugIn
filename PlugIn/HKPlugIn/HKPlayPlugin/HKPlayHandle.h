...
#ifndef HKPlayHandle_h__
#define HKPlayHandle_h__
...
public:
    ...
    static void CALLBACK SaveToBmpFile(LONG nPort,char * pBuf,long nSize,long nWidth,long nHeight,long nStamp,long nType,void *nReserved);

    static void  CALLBACK DrawFunCallBack(long lRealHandle,HDC hDc, void *dwUser);

    static void CALLBACK DecCBFun4Standard(long nPort, char * pBuf, long nSize, FRAME_INFO * pFrameInfo, void *nReserved1,void *nReserved2);
	...
	static void CALLBACK pHKFileOverCB(long nPort, void *pUser);
	...
	typedef struct LABEL
	{
		...
		//byte type;
		//byte res0; // :1 预留字段
		unsigned short viewAngleH; //:2 水平张角 放大100 倍
		unsigned short viewAngleV; //:2 垂直张角 放大100 倍
		unsigned short r; //:2 可视域半径 扩大10 倍 单位米
						  //unsigned short r_max;
		int north; //:2 当前朝向，偏移指北度数 放大100 倍
				   //[MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 8, ArraySubType =UnmanagedType.I1)]
				   //byte[] res1; // :8 预留字段
	}AdditionDataInfo_D;

	static void CALLBACK AdditionData4SCBFun(long nPort, AdditionDataInfo* pstAddDataInfo, void* pUser);
public:
	int m_nSrcWind;
	int m_nSrcWindEx;
	LONG m_nSrcWidth;
    ...
    int m_nMaxDisplayBuf;	//最大SDK播放缓冲区
    ...
    HANDLE m_hLocPlayHandle;

    int m_nUseOne4MultScreen;  //是否使用一机多频 0-否 1-是
	...
	static CRITICAL_SECTION m_picLock;

    CParamRead m_pam;
private:
    int m_streamType; //0=DVR实时流; 1=DVR文件流
    WriteLog m_log;

protected:
    list<pair<BYTE*,int> > m_listBuffer;
    bool m_nPuse;
    bool m_read;
...
/************************************************************************/
    ...
    /*                           公用接口                                   */
    /************************************************************************/
    bool queryPorperySupport(XYPROPROPERTY );
    //获取错误代码
    virtual DWORD getLastError(){return 0;};
};
#endif // HKPlayHandle_h__